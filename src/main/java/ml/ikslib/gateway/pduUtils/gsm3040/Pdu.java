...
package ml.ikslib.gateway.pduUtils.gsm3040;
...
import ml.ikslib.gateway.pduUtils.gsm3040.ie.ConcatInformationElement;
import ml.ikslib.gateway.pduUtils.gsm3040.ie.InformationElement;
import ml.ikslib.gateway.pduUtils.gsm3040.ie.PortInformationElement;
...
public abstract class Pdu {
	...
	public void setTpMti(int value) {
		setFirstOctetField(PduUtils.TP_MTI_MASK, value, new int[] { PduUtils.TP_MTI_SMS_DELIVER,
				PduUtils.TP_MTI_SMS_STATUS_REPORT, PduUtils.TP_MTI_SMS_SUBMIT });
	}
	...
	public boolean isBinary() {
		...
		// Changed following line according to
		// http://code.google.com/p/smslib/issues/detail?id=187
	...
	public final String toString() {
		...
		if (this.smscAddress != null) {
			sb.append("SMSC Address: [Length: " + getSmscInfoLength() + " ("
					+ PduUtils.byteToPdu((byte) getSmscInfoLength()) + ") octets");
			sb.append(", Type: " + PduUtils.byteToPdu(this.smscAddressType) + " ("
					+ PduUtils.byteToBits((byte) this.smscAddressType) + ")");
		...
		if (this.udData != null) {
			...
			case PduUtils.DCS_ENCODING_7BIT:
				sb.append(
						"User Data Length: " + getUDLength() + " (" + PduUtils.byteToPdu(getUDLength()) + ") septets");
				sb.append("\n");
				break;
			case PduUtils.DCS_ENCODING_8BIT:
			case PduUtils.DCS_ENCODING_UCS2:
