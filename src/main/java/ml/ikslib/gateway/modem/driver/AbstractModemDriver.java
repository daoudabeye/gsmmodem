...
import ml.ikslib.gateway.modem.DeviceInformation;
...
import ml.ikslib.gateway.modem.ModemResponse;
...
public abstract class AbstractModemDriver {
    ...
    Pattern CMTI_REGEX = Pattern.compile("^\\+CMTI:\\s*\"([^\"]+)\",\\s*(\\d+)$");
    ...
    protected ArrayList<CnmiCallbackEvent> cnmi = new ArrayList<>();
    ...
    public ModemResponse write(String data)
            throws IOException, TimeoutException, NumberFormatException, InterruptedException {
        return write(data, false);
    }

    public ModemResponse write(String data, boolean skipResponse)
            throws IOException, TimeoutException, NumberFormatException, InterruptedException {
    ...
    protected void write(byte[] s) throws IOException, NumberFormatException {
        int charDelay = Integer.parseInt(getModemSettings("char_wait_unit"));
        if (charDelay == 0)
            this.out.write(s);
        else {
    ...
    private String getResponse() throws TimeoutException, IOException, NumberFormatException, InterruptedException {
        ...
        while (true) {
            ...
            if (line.startsWith("+STIN:"))
                continue;
            if (Integer.parseInt(getModemSettings("cpin_without_ok")) == 1) {
            ...
            if (line.startsWith("+CUSD")) {
                ussdResponse = true;
                this.responseOk = true;
                if (line.indexOf(",15") >= 0 || line.startsWith("+CUSD: 4")) {
                    raw.append(line);
                    raw.append("$");
                    b.append(line);
                    ussdResponse = false;
            ...
            if (line.indexOf(",15") >= 0 && ussdResponse) {
                raw.append(line);
                raw.append("$");
                b.append(line);
                ussdResponse = false;
            ...
            if (line.startsWith("+CLIP:")) {
                ...
                if (m.find())
                    callerId = m.group(1).replaceAll("\"", "");
                Service.getInstance().getCallbackManager().registerInboundCallEvent(new MsIsdn(callerId),
                        this.modem.getGatewayId());
                Common.countSheeps(Integer.valueOf(getModemSettings("wait_unit")));
                continue;
            }
            ...
            if ((line.indexOf("ERROR") == 0) || (line.indexOf("+CMS ERROR") == 0)
                ...
                logger.error(getPortInfo() + " ERR==> " + line + "AT==> " + lastAt);
...
//			System.err.println("Buffer :" + buffer );
            endTimeout += Integer.parseInt(getModemSettings("wait_unit"));
            if ((endTimeout - startTimeout) > Integer.parseInt(getModemSettings("timeout")))
                throw new TimeoutException("Timeout elapsed for " + getPortInfo());
            Common.countSheeps(Integer.parseInt(getModemSettings("wait_unit")));
        }
    ...
    public class PollReader extends Thread {
        ...
        public void run() {
            ...
            while (!this.shouldCancel) {
                ...
                try {
                    Common.countSheeps(Integer.parseInt(getModemSettings("poll_reader")));
                } catch (Exception e) {
                    logger.error(e.getMessage(), e);
                }
            }
    ...
    public void initializeModem() throws Exception {
        ...
        synchronized (this._LOCK_) {
            ...
            Common.countSheeps(Integer.parseInt(getModemSettings("wait_unit"))
                    * Integer.parseInt(getModemSettings("delay_after_init1")));
            atFromModemSettings("init2");
            Common.countSheeps(Integer.parseInt(getModemSettings("wait_unit"))
                    * Integer.parseInt(getModemSettings("delay_after_init2")));
            clearResponses();
            atEchoOff();
            clearResponses();
            atFromModemSettings("pre_pin");
            Common.countSheeps(Integer.parseInt(getModemSettings("wait_unit"))
                    * Integer.parseInt(getModemSettings("delay_after_pre_pin")));
            while (true) {
                counter++;
                if (counter == 5)
                    throw new IOException("Modem does not correspond correctly, giving up...");
                ModemResponse simStatus = atGetSimStatus();
                if (simStatus.getResponseData().indexOf("SIM PIN2") >= 0) {
                    if (Common.isNullOrEmpty(this.modem.getSimPin2()))
                        throw new IOException("SIM PIN2 requested but not defined!", null);
                    atEnterPin(this.modem.getSimPin2());
                } else if (simStatus.getResponseData().indexOf("SIM PIN") >= 0) {
                    if (Common.isNullOrEmpty(this.modem.getSimPin()))
                        throw new IOException("SIM PIN requested but not defined!", null);
                ...
                Common.countSheeps(Integer.parseInt(getModemSettings("wait_unit"))
                        * Integer.parseInt(getModemSettings("delay_on_sim_error")));
            }
            atFromModemSettings("post_pin");
            Common.countSheeps(Integer.parseInt(getModemSettings("wait_unit"))
                    * Integer.parseInt(getModemSettings("delay_after_post_pin")));
            atEnableClip();
            if (!atNetworkRegistration().isResponseOk())
                throw new IOException("Network registration failed!", null);
            atVerboseOff();
            if (atSetPDUMode().isResponseOk())
                ...
                if (atSetTEXTMode().isResponseOk()) {
                    ...
                    throw new IOException("Neither PDU nor TEXT mode are supported by this modem!");
            }
...
//            atCnmiOff();
    ...
    void retrieveMemoryLocations() throws IOException {
        if (Common.isNullOrEmpty(this.memoryLocations)) {
                ...
                try {
                    ...
                    if (response.indexOf("+CPMS:") >= 0) {
                        ...
                        while (tokens.hasMoreTokens()) {
                            ...
                            if ((!loc.equalsIgnoreCase("MT")) && ((this.memoryLocations.indexOf(loc) < 0)))
            ...
            logger.info("Using given memory locations: " + this.memoryLocations);
    }
    ...
    protected ModemResponse atUssdResultEnable() throws Exception {
        return write("AT+CUSD=1\r", true);
    }
    ...
    protected ModemResponse atCnmi() throws Exception {
        return write("AT+CNMI=2,1,0,2\r");
    }
    ...
    protected ModemResponse atCnmiOffV2() throws Exception {
        return write("AT+CNMI=2,0,0,0\r");
    }
...
//		write("AT+CUSD=1,\"*777#\",15", true);
    ...
    public int atSendTEXTMessage(String recipient, String text) throws Exception {
        ...
        if (!this.responseOk)
            throw new Exception("Unsupported encoding: " + this.modem.getDeviceInformation().getEncoding());
        write(String.format("AT+CMGS=\"%s\"\r", recipient), true);
        while (this.buffer.length() == 0)
            Common.countSheeps(Integer.parseInt(getModemSettings("wait_unit")));

        Common.countSheeps(Integer.parseInt(getModemSettings("wait_unit"))
                * Integer.parseInt(getModemSettings("delay_before_send_pdu")));
        clearResponses();
        write(text, true);
        write((byte) 26);
        String response = getResponse();
    ...
    public USSDResponse atSendUssd(String rawRequest, boolean interactive) throws Exception {
        logger.debug("ussd request: " + rawRequest);
    ...
    public ModemResponse readStoredSms(String msgIndex, String msgMemory) throws Exception {
        if (atSwitchMemoryLocation(msgMemory).isResponseOk()){
            return write(String.format("AT+CMGR=\"%s\"\r", msgIndex));
        }
        return new ModemResponse("", false);
    }

    public ModemResponse atGetMessages() throws Exception {
        return (this.modem.getDeviceInformation().getMode() == Modes.PDU ? write("AT+CMGL=4\r")
                : write("AT+CMGL=\"ALL\"\r"));
    }
    ...
    public String getModemSettings(String key) throws IOException {
        ...
        String value = "";
        if (!Common.isNullOrEmpty(fullSignature))
            value = modemProperties.getProperty(fullSignature + "." + key);
        if ((Common.isNullOrEmpty(value)) && (!Common.isNullOrEmpty(shortSignature)))
    ...
    public ArrayList<CnmiCallbackEvent> getCnmi(){
        return cnmi;
    }

    public void clearCnmi(){
        cnmi.clear();
    }
}
