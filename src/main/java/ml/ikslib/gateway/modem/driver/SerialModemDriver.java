...
import ml.ikslib.gateway.modem.driver.serial.SerialPortEvent;
import ml.ikslib.gateway.modem.driver.serial.SerialPortEventListener;
...
public class SerialModemDriver extends AbstractModemDriver implements SerialPortEventListener {
	...
	public void openPort() throws NumberFormatException, IOException {
		...
		else if (getModemSettings("flowcontrol").equalsIgnoreCase("IN"))
			this.serialPort.setFlowControlMode(SerialPort.FLOWCONTROL_RTSCTS_IN);
		else if (getModemSettings("flowcontrol").equalsIgnoreCase("OUT"))
			this.serialPort.setFlowControlMode(SerialPort.FLOWCONTROL_RTSCTS_OUT);
