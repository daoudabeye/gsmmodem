...
import ml.ikslib.gateway.modem.client.common.AbstractGsmModemClient;

public abstract class JAbstractModemDriver extends AbstractGsmModemClient {
	...
	public JAbstractModemDriver(Modem modem) {
			...
			while (!this.shouldCancel) {
				try {
					while (hasData()) {
						char c = (char) read();
						// logger.debug("> " + c);
						JAbstractModemDriver.this.buffer.append(c);
						if (JAbstractModemDriver.this.buffer.indexOf("+CLIP") >= 0) {
							if (!this.foundClip) {
								this.foundClip = true;
