...
import ml.ikslib.gateway.message.InboundBinaryMessage;
import ml.ikslib.gateway.message.InboundEncryptedMessage;
import ml.ikslib.gateway.message.InboundMessage;
import ml.ikslib.gateway.message.Payload;
import ml.ikslib.gateway.modem.DeviceInformation.Modes;
import ml.ikslib.gateway.pduUtils.gsm3040.Pdu;
import ml.ikslib.gateway.pduUtils.gsm3040.PduParser;
import ml.ikslib.gateway.pduUtils.gsm3040.PduUtils;
import ml.ikslib.gateway.pduUtils.gsm3040.SmsDeliveryPdu;
import ml.ikslib.gateway.pduUtils.gsm3040.SmsStatusReportPdu;
...
public class MessageReader extends Thread {
    ...
    BlockingQueue<CnmiCallbackEvent> blockingQueue = new LinkedBlockingDeque<>(50);
            ...
            if (this.modem.getStatus() == Status.Started) {
                try {
                    synchronized (this.modem.getModemDriver()._LOCK_) {
                        CnmiCallbackEvent cnmiCallbackEvent = blockingQueue.take();
                        String msgMemory = cnmiCallbackEvent.getMsgMemory();
                        String msgIndex = cnmiCallbackEvent.getMsgIndex();
                        String smsData = this.modem.getModemDriver().readStoredSms(msgIndex, msgMemory).getResponseData();
                        if (smsData.length() > 0) {
                            ArrayList<InboundMessage> messageList = (this.modem.getDeviceInformation()
                                    .getMode() == DeviceInformation.Modes.PDU ? parsePDU(smsData, msgMemory)
                                    : parseTEXT(smsData, msgMemory));
                            for (InboundMessage message : messageList) {
                                this.modem.getMessageReader().processMessage(message);
                                this.modem.getModemDriver().atDeleteMessage(msgMemory, Integer.parseInt(msgIndex));
//                                logger.info(message.toShortString());
                            }
                        }
                    }
                } catch (Exception e) {
                    logger.error("Unhandled exception! Modem :" + this.modem.getGatewayId(), e);
                }
            }
    ...
    private ArrayList<InboundMessage> parsePDU(String data, String memLocation) throws IOException {
        logger.info("pdu data " + data);
        logger.info("memLocation " + data);
        ...
        while (true) {
            ...
            if (pdu instanceof SmsDeliveryPdu) {
                ...
                if (pdu.isBinary()) {
                    ...
                    if (Service.getInstance().getKeyManager().getKey(msg.getOriginatorAddress()) != null)
                        msg = new InboundEncryptedMessage((SmsDeliveryPdu) pdu, memLocation, memIndex);
                } else {
                    msg = new InboundMessage((SmsDeliveryPdu) pdu, memLocation, memIndex);
                }
                msg.setGatewayId(this.modem.getGatewayId());
                msg.setGatewayId(this.modem.getGatewayId());
                logger.debug("IN-DTLS: MI:" + msg.getMemIndex() + " REF:" + msg.getMpRefNo() + " MAX:"
                        + msg.getMpMaxNo() + " SEQ:" + msg.getMpSeqNo());
                if (msg.getMpRefNo() == 0)
                    messageList.add(msg);
                else {
                    // multi-part message
        ...
        for (List<InboundMessage> remainingList : mpMsgList) {
            for (InboundMessage msg : remainingList) {
                if (Common.getAgeInHours(msg.getSentDate()) > Settings.hoursToRetainOrphanedMessageParts) {
                    ...
                    // if (Service.getInstance().getOrphanedMessageNotification() != null) if
                    // (Service.getInstance().getOrphanedMessageNotification().process(Service.getInstance().getGateway(msg.getGatewayId()),
                    // msg) == true) deleteMessage(msg);
                    //
                }
            }
        }
        ...
        try {
            synchronized (this.modem.getModemDriver()._LOCK_) {
                for (int i = 0; i < (this.modem.getModemDriver().getMemoryLocations().length() / 2); i++) {
                    ...
                    if (data.length() > 0) {
                        ArrayList<InboundMessage> messageList = (this.modem.getDeviceInformation()
                                .getMode() == Modes.PDU ? parsePDU(data, memLocation)
                                : parseTEXT(data, memLocation));
                        for (InboundMessage message : messageList){
                            processMessage(message);
                            this.modem.getModemDriver().atDeleteMessage(message.getMemLocation(), message.getMemIndex());
                        }
                    }
                }
            }
            ...
            boolean s = Service.getInstance().getGatewayById(this.modem.getGatewayId()).stop();
            if (s)
                Service.getInstance().getGatewayById(this.modem.getGatewayId()).start();
        } catch (Exception e) {
            logger.error("Unhandled exception! Modem :" + this.modem.getGatewayId(), e);
        }
    }

    public void addCnmiEvent(CnmiCallbackEvent event) {
