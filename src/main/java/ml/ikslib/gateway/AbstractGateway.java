			...
			if ((getStatus() == Status.Stopped) || (getStatus() == Status.Error)) {
				try {
					...
					for (int i = 0; i < this.gatewayMessageDispatchers.length; i++) {
						this.gatewayMessageDispatchers[i] = new GatewayMessageDispatcher(
								String.format("Gateway Dispatcher %d [%s]", i, getGatewayId()), getMessageQueue(),
								this);
	...
	final public boolean send(OutboundMessage message) throws Exception {
		...
		try {
			if (getStatus() != Status.Started) {
				logger.warn("Outbound message routed via non-started gateway: " + message.toShortString() + " ("
						+ getStatus() + ")");
				return false;
			}
	...
	final public USSDResponse send(USSDRequest command) throws Exception {
		...
		try {
			...
			if (result != null) {
				...
				getStatistics().increaseTotalUssdFailed();
				Service.getInstance().getStatistics().increaseTotalUssdFailed();
			}
			return result;
		} catch (Exception e) {
			getStatistics().increaseTotalUssdFailures();
			Service.getInstance().getStatistics().increaseTotalUssdFailures();
			throw e;
		} finally {
			if (acquiredLock)
				this.concurrency.release();
