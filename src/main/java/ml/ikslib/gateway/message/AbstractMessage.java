...
public abstract class AbstractMessage implements Serializable {
	public enum Encoding {
		...
		public static Encoding getEncodingFromShortString(String shortString) {
			if (shortString.equalsIgnoreCase("7"))
				return Enc7;
			else if (shortString.equalsIgnoreCase("8"))
				return Enc8;
			else if (shortString.equalsIgnoreCase("U"))
				return EncUcs2;
			else if (shortString.equalsIgnoreCase("C"))
				return EncCustom;
			else
				return null;
		}
	...
	public enum Type {
		Inbound, Outbound, StatusReport, Ussd
	}
	...
	protected MsgType msgType = MsgType.SMS;
	...
	public String toString() {
		...
		if (getPayload() != null) {
			...
			b.append(String.format("Text payload: %s%n",
					(getPayload().getText() == null ? "null" : getPayload().getText())));
			b.append(String.format("Binary payload: %s%n",
		...
		if (this instanceof InboundEncryptedMessage) {
			try {
				b.append(String.format("Decrypted binary payload: %s%n",
