...
import ml.ikslib.gateway.pduUtils.gsm3040.PduFactory;
import ml.ikslib.gateway.pduUtils.gsm3040.PduGenerator;
import ml.ikslib.gateway.pduUtils.gsm3040.PduParser;
import ml.ikslib.gateway.pduUtils.gsm3040.PduUtils;
import ml.ikslib.gateway.pduUtils.gsm3040.SmsSubmitPdu;
import ml.ikslib.gateway.pduUtils.gsm3040.ie.InformationElementFactory;
...
public class OutboundMessage extends AbstractMessage {
	...
	public enum FailureCause {
		None("00"), BadNumber("01"), BadFormat("02"), GatewayFailure("03"), AuthFailure("04"), NoCredit("05"),
		OverQuota("06"), NoRoute("07"), Unavailable("08"), HttpError("09"), UnknownFailure("10"), Cancelled("11"),
		NoService("12"), MissingParms("13");
		private final String shortString;
	...
	protected void initPduObject(SmsSubmitPdu pdu, MsIsdn smscNumber) {
		...
		if (getEncoding() == Encoding.Enc8)
			pdu.setDataBytes(getPayload().getBytes());
		else
			pdu.setDecodedText(getPayload().getText());
	}
	...
	public String getPduUserData(boolean extRequestDeliveryReport) {
		...
		// - if the user wishes to extract the UD per part, he would need to get all
		// pduStrings
		// using getPdus(String smscNumber, int mpRefNo), use a
		// PduParser on each pduString in the returned list, then access the UD via the
		// Pdu object
		List<String> pdus = pduGenerator.generatePduList(pdu, 1);
		// my this point, pdu will be updated with concat info (in udhi), if present
		if ((pdu.hasTpUdhi()) && (getEncoding() == Encoding.Enc7)) {
	...
	public String getPduUserDataHeader(boolean extRequestDeliveryReport) {
		...
		initPduObject(pdu, new MsIsdn());
		// NOTE: - the mpRefNo is arbitrarily set to 1
		// - if the user wishes to extract the UDH per part, he would need to get all
		// pduStrings
		// using getPdus(String smscNumber, int mpRefNo), use a
		// PduParser on each pduString in the returned list, then access the UDH via the
		// Pdu object
		List<String> pdus = pduGenerator.generatePduList(pdu, 1);
