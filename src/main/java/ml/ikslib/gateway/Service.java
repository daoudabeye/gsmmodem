...
import ml.ikslib.gateway.crypto.KeyManager;
import ml.ikslib.gateway.groups.Group;
import ml.ikslib.gateway.groups.GroupManager;
...
import ml.ikslib.gateway.hook.IRouteHook;
...
import ml.ikslib.gateway.routing.AbstractBalancer;
import ml.ikslib.gateway.routing.AbstractRouter;
import ml.ikslib.gateway.routing.DefaultBalancer;
import ml.ikslib.gateway.routing.DefaultRouter;
import ml.ikslib.gateway.threading.CallbackManager;
import ml.ikslib.gateway.threading.ServiceMessageDispatcher;
        ...
        synchronized (this._LOCK_) {
            if (getStatus() == Status.Stopped) {
                ...
                if (allStarted)
                    ...
                    logger.warn("Service started, but some gateways did not start!");
            }
        }
        return allStarted;
    }
        ...
        synchronized (this._LOCK_) {
            if (getStatus() == Status.Started) {
                ...
                if (allStopped)
                    ...
                    logger.warn("Service stopped, but some gateways did not stop!");
            }
        }
        return allStopped;
    }
...
//		this.httpServer.registerHttpRequestACL(path, cidr);
                ...
                if (!selectedGateways.isEmpty()) {
                    for (int i = 0; i < selectedGateways.size(); i++) {
                        ...
                        try {
                            ...
                            logger.error("Unhandled Exception ! Gateway : " + gateway.getGatewayId(), e);
                        }
                    }
    ...
    public USSDResponse send(USSDRequest ussd) {
        logger.debug("Send: " + ussd.toString());
        USSDResponse response = null;
        if (getStatus() == Status.Started) {
            Collection<AbstractGateway> routes = routeUssdRequest(ussd);
            if (!routes.isEmpty()) {
                LinkedList<AbstractGateway> selectedGateways = new LinkedList<>(routes);
                if (!selectedGateways.isEmpty()) {
                    for (int i = 0; i < selectedGateways.size(); i++) {
                        AbstractGateway gateway = selectedGateways.get(i);
                        logger.debug("Trying ussd sending via: " + gateway.getGatewayId());
                        try {
                            response = gateway.send(ussd);
                            if (response != null)
                                break;
                        } catch (Exception e) {
                            logger.error("Unhandled Exception!", e);
                    ...
                    if (ussd.getSentStatus() != SentStatus.Sent)
                        ussd.setSentStatus(SentStatus.Failed);
                } else {
                    ussd.setSentStatus(SentStatus.Failed);
                    ussd.setFailureCause(FailureCause.NoRoute);
                }
            } else {
                ussd.setSentStatus(SentStatus.Failed);
                ussd.setFailureCause(FailureCause.NoRoute);
            ...
            ussd.setSentStatus(SentStatus.Failed);
            ussd.setFailureCause(FailureCause.NoService);
        }
        return response;
    }
    ...
    public boolean unregisterGateway(AbstractGateway gateway) {
        synchronized (this._LOCK_) {
            ...
            if ((getStatus() == Status.Started) || (gateway.getStatus() == AbstractGateway.Status.Started)) {
                ...
                if (!startStatus)
                    logger.warn(String.format("Gateway %s did not stop!", gateway.getGatewayId()));
                return startStatus;
            }
            return true;
        }
    ...
    public Collection<AbstractGateway> routeUssdRequest(USSDRequest ussd) {
        try {
            Collection<AbstractGateway> selectedGateways = getRouter().route(ussd, getGateways().values());
            selectedGateways = getBalancer().balance(ussd, selectedGateways);
            return selectedGateways;
        } catch (ConcurrentModificationException e) {
            logger.warn("Gateway list modified, retrying routing...", e);
            return routeUssdRequest(ussd);
        }
    }
